...
`.flow` 是一种用于描述流程图语义的轻量级声明式 DSL。它以纯文本形式定义节点和有向边，支持方向配置、节点分组和带标签的连线；稳定位置、分组盒子和边挂靠侧保存在同名的 `.flow.layout.json` sidecar 中。
...
edge start -> end
edge review -> done "通过"
edge review -> revise "驳回" dashed id=e_review_pass
...
**语法：**
```
edge <起点ID> -> <终点ID> ["<连线标签>"] [dashed|dotted|dashdot] [id=<边ID>]
```
...
- **`id=<边ID>`**：可选但推荐，提供稳定边身份；旧文件打开后编辑器会自动补齐
...
edge start -> build
edge build -> llm
edge llm -> decision id=e_llm_decision
...
| 手动布局 | 通过 `.flow.layout.json` 持久化 | 不支持稳定 sidecar 布局 |
