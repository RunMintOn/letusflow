...
test('app defaults node dragging to off and wires the toggle into flow canvas', async () => {
  const source = await readFile('src/webview-app/App.jsx', 'utf8')

  assert.match(source, /const \[isNodeDraggingEnabled, setIsNodeDraggingEnabled\] = React\.useState\(false\)/)
